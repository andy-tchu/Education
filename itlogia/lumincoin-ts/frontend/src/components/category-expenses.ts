...
export class CategoryExpenses {
    ...
    constructor() {
        const that: CategoryExpenses = this;
    ...
    private showCategories(): void {
        if (this.categories) {
            this.categories.forEach((category: CategoryType) => {
                ...
                const categoryEditElement: HTMLElement = ElementHelper.createElement('a', 'btn btn-primary', 'Редактировать',
                    {'href': '#/category-expenses-edit'});
                categoryEditElement.onclick = function () {
                    const parentElement: HTMLElement | null = (this as HTMLElement).parentElement;
                    if (parentElement) {
                ...
                const categoryDeleteElement: HTMLElement = ElementHelper.createElement('a', 'btn btn-danger', 'Удалить', {
        ...
        const categoryElement: HTMLElement = ElementHelper.createElement('div', 'card mb-3 me-3');
        categoryElement.innerHTML = "<a href=\"#/category-expenses-create\" class=\"card-body card-add\">\n" +
    ...
    private async deleteCategory(): Promise<void> {
        if (this.currentCategory) {
            try {
                const result: DefaultResponseType = await CustomHttp.request(config.host + '/categories/expense/' + this.currentCategory, 'DELETE');
