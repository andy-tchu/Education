...
export class Test {
    ...
    private chooseAnswer(): void {
        if (this.nextButtonElement && this.passButtonElement) {
            ...
            const passImageElement = document.getElementById('pass-image');
            if (passImageElement) {
                passImageElement.setAttribute('src', '/images/small-arrow-grey.png');
            }
    ...
    noChooseAnswer() {
        if (this.nextButtonElement && this.passButtonElement) {
            ...
            const passImageElement:HTMLElement | null = document.getElementById('pass-image');
            if (passImageElement) {
                passImageElement.setAttribute('src', '/images/small-arrow.png');
            }
            this.passButtonElement.onclick = this.move.bind(this, ActionTestType.pass);
        }
    }
    ...
    private async complete(): Promise<void> {
        ...
        try {
            const result: DefaultResponseType | PassTestResponseType = await CustomHttp.request(config.host + '/tests/' + id + '/pass', 'POST',
                {
