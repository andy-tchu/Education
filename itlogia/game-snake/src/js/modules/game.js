...
export class Game {
    ...
    showGrid() {
        ...
        for (let i = 0; i <= size; i += this.positionsSize) {
            this.context.moveTo(0.5 + i + this.positionsSize, 0);
            this.context.lineTo(0.5 + i + this.positionsSize, size + this.positionsSize)
        }

        for (let i = 0; i <= size; i += this.positionsSize) {
            this.context.moveTo(0, 0.5 + i + this.positionsSize);
            this.context.lineTo(size + this.positionsSize, 0.5 + i + this.positionsSize)
        }
